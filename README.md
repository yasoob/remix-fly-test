...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-fly-test-f82f
  fly apps create remix-fly-test-f82f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-fly-test-f82f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-fly-test-f82f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-fly-test-f82f-staging' does not match app name in config file 'remix-fly-test-f82f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-fly-test-f82f-db
  fly postgres attach --app remix-fly-test-f82f remix-fly-test-f82f-db

  fly postgres create --name remix-fly-test-f82f-staging-db
  fly postgres attach --app remix-fly-test-f82f-staging remix-fly-test-f82f-staging-db
  ```
